...
try {
  ...
  if ($conn) {
    ...
    $stmt = $conn->prepare("SELECT id_buku, cover_url, judul, tanggal_upload, status_verifikasi FROM buku WHERE id_penulis = ? ORDER BY tanggal_upload DESC"); // Perubahan di sini
